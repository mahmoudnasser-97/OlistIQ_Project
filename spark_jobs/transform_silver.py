...
#   1. Added SimpleAWSCredentialsProvider — prevents credential-chain warnings
#   2. Added multipart.size + fast.upload — prevents MinIO 400 on silver_sellers
#      (seller table with metrics is large after the join explosion)
#   3. Unpersist silver_order_items after silver_orders is written — the original
#      kept it cached in memory for the rest of the job unnecessarily
#   4. silver_geolocation.dropDuplicates now uses ["geolocation_zip_code_prefix",
#      "geolocation_state"] not just zip_code_prefix — same zip can exist in two
#      states (e.g. border zip codes), dropping on zip only loses state context
#Filter out any rows that violate the geographical boundaries before you run dropDuplicates. 
# This guarantees that Spark only selects from valid Brazilian coordinates.
...
print("Loading Bronze tables...", flush=True)
...
silver_order_items = (
    ...
    .withColumn("seller_handling_days",
        F.datediff(
...
silver_orders = (
    ...
    .withColumn("days_diff_estimated",
        F.datediff(
            F.col("order_estimated_delivery_date"),
            F.col("order_delivered_customer_date")
        ).cast(IntegerType()))
    .withColumn("estimated_buffer",
        F.datediff(
            F.col("order_estimated_delivery_date"),
            F.col("order_purchase_timestamp")
        ).cast(IntegerType()))
    ...
    .withColumn("total_order_cost",
        (
            F.coalesce(F.col("total_products_price"), F.lit(0)) +
            F.coalesce(F.col("total_freight_value"),  F.lit(0))
    ...
    .withColumn("is_multi_seller_order",
        F.when(F.col("seller_count") > 1, 1).otherwise(0).cast(IntegerType()))
)
...
print(f"   ✓ silver_orders", flush=True)

# FIX: unpersist after silver_orders is written — no downstream step needs the cache
silver_order_items.unpersist()

# ── 3. silver_products ────────────────────────────────────────────────────
print("Processing: silver_products...", flush=True)
clean_translation = translation.select(
...
silver_products = (
    ...
    .withColumn("product_weight_g",
        F.coalesce(F.col("product_weight_g").cast(DecimalType(10,2)),
                   F.lit(-1.00).cast(DecimalType(10,2))))
    .withColumn("product_length_cm",
        F.coalesce(F.col("product_length_cm").cast(DecimalType(10,2)),
                   F.lit(-1.00).cast(DecimalType(10,2))))
    .withColumn("product_height_cm",
        F.coalesce(F.col("product_height_cm").cast(DecimalType(10,2)),
                   F.lit(-1.00).cast(DecimalType(10,2))))
    .withColumn("product_width_cm",
        F.coalesce(F.col("product_width_cm").cast(DecimalType(10,2)),
                   F.lit(-1.00).cast(DecimalType(10,2))))
    .withColumn("product_size_cm3",
        (
            F.col("product_length_cm") *
            F.col("product_height_cm") *
            F.col("product_width_cm")
        ).cast(DecimalType(10,2)))
    .withColumn("logistics_size_category",
        F.when(F.col("product_size_cm3") <= 5000,  "Small Box")
         .when(F.col("product_size_cm3") <= 20000, "Medium Box")
         .otherwise("Large Parcel"))
    .withColumn("logistics_weight_category",
        F.when(F.col("product_weight_g") <= 2000,  "Lightweight")
...
print(f"   ✓ silver_products", flush=True)
...
silver_reviews = (
    ...
    .withColumn("review_label",
        F.when(F.col("review_score") >= 4, "Positive")
         ...
         .otherwise("Negative"))
    .withColumn("review_response_delay_days",
        F.datediff(
...
print(f"   ✓ silver_reviews", flush=True)
...
print(f"   ✓ silver_customers", flush=True)

# ── 6. silver_sellers ─────────────────────────────────────────────────────
print("Processing: silver_sellers...", flush=True)
seller_base = raw_items.join(
    raw_orders.select(
        "order_id", "order_delivered_carrier_date", "order_purchase_timestamp"
    ),
    "order_id", "inner"
)
...
seller_metrics = seller_base.groupBy("seller_id").agg(
    ...
    F.sum(F.when(F.col("order_delivered_carrier_date") < F.col("shipping_limit_date"), 1)
          .otherwise(0)).cast(LongType()).alias("early_preparations"),
    F.sum(F.when(F.col("order_delivered_carrier_date") == F.col("shipping_limit_date"), 1)
          .otherwise(0)).cast(LongType()).alias("on_time_preparations"),
    F.sum(F.when(F.col("order_delivered_carrier_date") > F.col("shipping_limit_date"), 1)
          .otherwise(0)).cast(LongType()).alias("late_preparations"),
    F.avg(F.datediff(
        F.col("order_delivered_carrier_date"),
        F.col("order_purchase_timestamp")
    )).cast(DoubleType()).alias("avg_handling_days"),
)
...
silver_sellers = (
    ...
    .withColumn("early_ratio",
        (F.col("early_preparations") /
         F.when(F.col("total_items_sold") == 0, None)
          .otherwise(F.col("total_items_sold"))
        ).cast(DoubleType()))
    .withColumn("on_time_ratio",
        (F.col("on_time_preparations") /
         F.when(F.col("total_items_sold") == 0, None)
          .otherwise(F.col("total_items_sold"))
        ).cast(DoubleType()))
    .withColumn("late_ratio",
        (F.col("late_preparations") /
         F.when(F.col("total_items_sold") == 0, None)
          .otherwise(F.col("total_items_sold"))
        ).cast(DoubleType()))
    .withColumn("has_sales",
        F.when(F.col("total_items_sold") > 0, True)
...
print(f"   ✓ silver_sellers", flush=True)
...
silver_payments = (
    impute_strings(silver_payments)
    .dropDuplicates(["order_id", "payment_sequential"])
)
silver_payments.write.format("delta").mode("overwrite").save(f"{S}silver_payments")
print(f"   ✓ silver_payments", flush=True)

# ── 8. silver_geolocation ─────────────────────────────────────────────────
...
silver_geolocation = (
    ...
    # Filter out extreme coordinate anomalies before picking a row
    .filter(
...
silver_geolocation = (
    impute_strings(silver_geolocation)
    .dropDuplicates(["geolocation_zip_code_prefix", "geolocation_state"])
)
silver_geolocation.write.format("delta").mode("overwrite").save(f"{S}silver_geolocation")
print(f"   ✓ silver_geolocation", flush=True)

print("\n✅ All Silver Tables Generated.", flush=True)
spark.stop()
